...
/**
* @dev Forked from: https://github.com/lorenzb/proveth/blob/master/onchain/ProvethVerifier.sol
*/
library ProvethVerifier {
    ...
    function validateMPTProof(bytes32 rootHash, bytes memory mptKey, RLP.RLPItem[] memory stack)
        internal
        pure
    ...
    function isEmptyBytesequence(RLP.RLPItem memory item) internal pure returns (bool) {
        if (item.len != 1) {
            return false;
        }
        uint8 b;
